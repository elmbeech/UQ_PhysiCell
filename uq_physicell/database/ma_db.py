...
def load_output(db_file: str, sample_ids: list = None, replicate_ids: list = None,
    ...
    try:
        ...
        if load_data:
            ...
            # Probe an (SampleID, ReplicateID) pair known to exist in this result -- not a hardcoded
            # (0, 0), which may not have completed yet (or exist at all, on a fresh/partial database)
            # even when other samples/replicates have, and would otherwise crash this lookup.
            if not df_output.empty:
                probe_sample_id, probe_replicate_id = (int(v) for v in df_output[['SampleID', 'ReplicateID']].iloc[0])
                df_output_test = load_output(db_file, sample_ids=[probe_sample_id], replicate_ids=[probe_replicate_id])
                ...
                if type(df_output_test['Data'].iloc[0]) == list:
                ...
                # else: Data holds some other already-extracted type (e.g. QoIs computed
                # directly by the summary function) -- no placeholder column is added.
